...
browser.forEach(browser => {
    describe('Hidden Layers', async function () {
        ...
        it("Check the green button attribute", async function () {
            this.timeout(timeoutTest)
            let button = await driver.getTheAttributeOfGreenButton()
            assert.equal(button,hiddenLayersDict.greenButtonIdAttr)
        })
        ...
        it.only("Click the green cannot hitted twice", async function () {
            this.timeout(timeoutTest)
